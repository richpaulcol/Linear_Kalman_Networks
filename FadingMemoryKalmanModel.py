...
Net.additional_BC_Uncertainty_Head(0.5) # % value of the initial value
...
np.random.seed(0)
MeasurementNoise = np.random.normal(0,0.01,[6,1000])
...
for i in Measurement_Nodes:
	MeasurementData.append(np.load('MeasureData'+str(i)+'.npy')+MeasurementNoise[int(i)-1,:])
...
for i in range(len(NotMeasured)):
	for j in Net.nodes:
		if j.Name == NotMeasured[i]:
			axs[1].plot(AllMeasuredData[int(NotMeasured[i])-1,:],label = j.Name+' Measured',marker='o',linewidth=0,color = colors[i])
